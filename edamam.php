...
class edamam implements databaseInterface {
	...
	//print "</pre>";
       // var_dump($Recipe_array['hits']); //print out array returned fortroubleshooting
    }
...
function nextRecipe($Recipe, $from) {
    ...
    private function createRecipeResultsArray(array $recipeArray) {
        foreach ($recipeArray['hits'] as $recipe) {
            ...
            $edamamRecipeID[] = $recipe['recipe']['uri'];
            $edamamRecipeTitle[] = $recipe['recipe']['label'];
        }
	return array("edamamRecipeURL" => $edamamRecipeURL, "edamamRecipeImageURL" => $edamamRecipeImageURL, "edamamRecipeID" => $edamamRecipeID, "edamamRecipeTitle" => $edamamRecipeTitle);
    }
}
